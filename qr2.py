import qrcode, PIL,csv
from PIL import Image,ImageTk
import tkinter as tk
from tkinter import ttk,messagebox,filedialog
with open(r"C:\Users\raman\Desktop\sample.csv", newline='') as csvfile:
...
def saveQR():
    ...
    if data:
        ...
        messagebox.showwarning("Warning",'Enter Data in Entry First')
def std(*args):
    data = int(text_entry.get())
    if (data>0) and (data<=63):
        img = qrcode.make(name[data]) #generate QRcode  
        res_img = img.resize((280,250)) # reszie QR Code Size
            #Convert To photoimage
        tkimage= ImageTk.PhotoImage(res_img)
        qr_canvas.create_image(0,0,anchor=tk.NW, image=tkimage)
        qr_canvas.image = tkimage
         ...
         messagebox.showwarning("Warning",'Entered wrong roll number.')
...
btn_4 = ttk.Button(frame2,text="student info",width=15,command=std)
btn_4.place(x=85,y=80)
